...
//It comes with node. Just require it. Node is like a mr potato head.
...
//part of node. No need to install. It gives node access to the file system
	//on THIS computer. The computer running the js file
const fs = require("fs");

//create service takes 2 args:
//1. the request
//2. the response
const server = http.createServer((req, res)=>{
	...
	console.log(req.url);
	if(req.url=== '/'){
		//writeHead takes minimum of 2 args:
		//1. status code
		//2. header object, which needs to include the mime-type of whatever is being sent back
		//mime-type describes the type of data (ex: text/plain)
...
// on port 8080;
//When a request comes in on port 8080 by http, the callback/anon function
//	in createServer will fire off with the req and res objects available
server.listen(8080);
